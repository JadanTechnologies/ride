...
export const DriverPortal: React.FC<DriverPortalProps> = ({ user, pricing, commissionRate, surge, dailyEarnings, history, withdrawals, onRideComplete, onWithdraw, onLogout, onNotify }) => {
    ...
    useEffect(() => {
        // Helper to safely execute actions based on current state
        const safeActions = {
            goOnline: () => {
            ...
            acceptRide: () => {
                if (activeRequest && rideStatus === 'idle') handleAcceptRide();
            },
            declineRide: () => {
                if (activeRequest && rideStatus === 'idle') setActiveRequest(null);
            },
            interactRide: () => {
                if (rideStatus !== 'idle') handleRideAction();
            }
        };

        // We need to re-register commands when state changes to capture closures
        // Or better, use refs, but for now simple re-registration works if efficient
        voiceCommandService.clearCommands();

        voiceCommandService.registerCommand({
            command: 'go online',
            keywords: ['online', 'go online', 'start working'],
            action: () => document.getElementById('btn-online')?.click()
        });

        voiceCommandService.registerCommand({
            command: 'go offline',
            keywords: ['offline', 'stop working', 'go offline'],
            action: () => document.getElementById('btn-offline')?.click()
        });

        voiceCommandService.registerCommand({
            command: 'accept ride',
            keywords: ['accept', 'confirm', 'yes', 'take it'],
            action: () => document.getElementById('btn-accept')?.click()
        });

        voiceCommandService.registerCommand({
            command: 'reject ride',
            keywords: ['reject', 'decline', 'no', 'cancel'],
            action: () => document.getElementById('btn-decline')?.click()
        });

        voiceCommandService.registerCommand({
            command: 'next step',
            keywords: ['arrived', 'start trip', 'start', 'complete', 'end trip', 'finish'],
            action: () => document.getElementById('btn-action')?.click()
        });

    }, [isOnline, activeRequest, rideStatus]); // Dependencies for re-registration logic if needed,
    // actually using DOM clicks avoids stale closures issues with simple service integration.
                        ...
                        <>
                            ...
                            {rideStatus === 'idle' && !activeRequest && (
                                <div className="absolute top-6 left-1/2 transform -translate-x-1/2 z-20">
                                    <button
                                        id={isOnline ? 'btn-offline' : 'btn-online'}
                            ...
                            {activeRequest && rideStatus === 'idle' && (
                                <div className="absolute bottom-6 left-4 right-4 md:left-1/2 md:transform md:-translate-x-1/2 md:w-[400px] bg-white rounded-2xl shadow-2xl p-6 animate-in slide-in-from-bottom-20 z-20">
                                    ...
                                    <div className="flex gap-3">
                                        <button id="btn-decline" onClick={() => setActiveRequest(null)} className="flex-1 py-3 font-bold text-gray-500 hover:bg-gray-100 rounded-xl">Decline</button>
                                        <button id="btn-accept" onClick={handleAcceptRide} className="flex-[2] py-3 bg-brand-600 text-white font-bold rounded-xl shadow-lg shadow-brand-500/30 hover:bg-brand-700">Accept Ride</button>
                                    </div>
                                </div>
                            )}
                                ...
                                <>
                                    ...
                                    <div className="absolute bottom-0 w-full bg-white rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.1)] p-6 z-20 animate-in slide-in-from-bottom-20">
                                        ...
                                        <div className="mt-6 flex justify-center">
                                            <PanicButton className="w-16 h-16 sm:w-20 sm:h-20" onPanic={() => onNotify('alert', 'EMERGENCY: Security team notified of your location!')} />
                                        </div>
                                    </div>
                                </>
                            )}
